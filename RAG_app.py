...
OPENAI_API_KEYS_URL = "https://platform.openai.com/docs/quickstart/make-your-first-api-request"
GOOGLE_API_KEYS_URL = "https://aistudio.google.com/"
OPENAI_MODEL_OPTIONS = ["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"]
GOOGLE_MODEL_OPTIONS = ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-2.5-flash-lite"]
HF_MODEL_OPTIONS = ["mistralai/Mistral-7B-Instruct-v0.3"]
COHERE_RERANK_MODEL = "rerank-v4.0-fast"

MODEL_OPTIONS_BY_PROVIDER = {
    "OpenAI": OPENAI_MODEL_OPTIONS,
    "Google": GOOGLE_MODEL_OPTIONS,
    "HuggingFace": HF_MODEL_OPTIONS,
}

API_KEY_PROMPTS_BY_PROVIDER = {
    "OpenAI": f"OpenAI API Key - [Get an API key]({OPENAI_API_KEYS_URL})",
    "Google": f"Google API Key - [Get an API key]({GOOGLE_API_KEYS_URL})",
    "HuggingFace": "HuggingFace API key - [Get an API key](https://huggingface.co/settings/tokens)",
}
...
def expander_model_parameters(
    ...
    if text_input_API_key is None:
        text_input_API_key = API_KEY_PROMPTS_BY_PROVIDER[LLM_provider]
    if list_models is None:
        list_models = MODEL_OPTIONS_BY_PROVIDER[LLM_provider]
...
def sidebar_and_documentChooser():
    ...
    with st.sidebar:
        ...
        if st.session_state.LLM_provider == "OpenAI" and st.session_state.selected_model.startswith("gpt-3.5"):
            # Legacy smaller-context OpenAI models skip the vectorstore-backed retriever.
                ...
                if selected_vectorstore_path == "":
                    ...
                    with st.spinner("Loading vectorstore..."):
                        ...
                        try:
                            ...
                            st.session_state.selected_vectorstore_name = Path(
                                selected_vectorstore_path
                            ).name
                            initialize_retrieval_chain(embeddings=embeddings)
                            st.info(
...
def select_embeddings_model():
    ...
    if st.session_state.LLM_provider == "Google":
        embeddings = GoogleGenerativeAIEmbeddings(
            model="gemini-embedding-001", google_api_key=st.session_state.google_api_key
        )
...
def initialize_retrieval_chain(embeddings):
    """Create the retriever, memory, and conversational chain from the current vectorstore."""
...
def create_retriever(
    ...
    Parameters:
        ...
        cohere_model (str): model used by Cohere, such as "rerank-v4.0-fast" or "rerank-v4.0-pro"
...
def CohereRerank_retriever(
    base_retriever, cohere_api_key, cohere_model=COHERE_RERANK_MODEL, top_n=10
):
    ...
    Parameters:
       ...
       cohere_model: the Cohere model, such as "rerank-v4.0-fast" or "rerank-v4.0-pro"
...
def create_memory(model_name="gpt-5.6-sol", memory_max_token=None):
    """Use summary memory for legacy smaller-context OpenAI models.
    Use a plain conversation buffer for current large-context models."""

    if model_name and model_name.startswith("gpt-3.5"):
